...
The practical consequence is substantial. A high modelled product wallet is no longer sufficient to enter the weekly plan. The system must show which business condition is supported, which stakeholder role owns the issue, which primary and supporting solutions are compatible, how the client may benefit, how the bank may benefit, whether operational gates permit more than discovery, whether a dated trigger exists, whether the candidate lies on a robust Pareto frontier, and which question could change the decision.

### V3.1 snapshot in numbers

| Implemented surface | Reproducible V3.1 result | Interpretation boundary |
|---|---:|---|
| Business Model Twins | 20 clients, 240 component records | Twelve records per client; unsupported values remain unknown |
| Typed business evidence | 905 claims and 71 explicit gaps | 85 E1 and 820 E0; 51 claims remain pending review |
| Business graphs | 993 nodes and 1,154 edges | Simulation/public-evidence topology, not an observed client treasury graph |
| Business events | 84 | Point-in-time, availability-dated inputs |
| Problem hypotheses | 360 across 18 problem types | 224 identified; 194 commercially eligible at signal level |
| Client-solution projections | 320 | 198 valid estimates; 122 fail closed with reasons |
| Conversation candidates | 224 | All are discovery-only because material bank feasibility is unknown |
| Weekly coverage plan | 8 selected, solver `OPTIMAL` | Pareto-filtered, constraint-compliant, downside-aware scenario policy |
| Active-learning questions | 891 evaluated; 308 positive-net-VOI selections | 512 common draws; answers remain pending E2 until approved |
| Governed event fabric | 1,148 events on four domain topics | Idempotent fixture event store; MSK is the production adapter |
| Measured competitor-share claims | 0 | E3 evidence is absent |
| Causal incremental-value claims | 0 | No randomized real-RM trial has been completed |

### Fundamental data contracts

`ConversationCandidate` is a strict, versioned contract. Its stable identifier hashes client, role, problem, bundle, window and snapshot version. It carries legal entities, a role-based stakeholder resolution, a problem interval with supporting and disconfirming evidence, a bundle of one primary and at most two supporting solutions, explicit 30/60/90-day timing, separate client and bank value, six feasibility gates, two Pareto memberships, weekly policy rank, a positive-net-VOI question when one exists, an evidence-backed explanation path, deterministic Why-How-What fields, artifact versions, claim classes, evidence tiers and an `as_of` date.

`BusinessTwinSnapshot` carries twelve domains: revenue, cost, working capital, funding, liquidity, operations, geography, currency/commodity exposure, projects/SPVs, stakeholder responsibilities, business/financial risks, and strategy/corporate actions/ESG. Every component is `SUPPORTED`, `INFERRED`, `UNKNOWN` or `NOT_APPLICABLE`. Each component retains typed facts, missing information, contradictions, materiality, freshness, time validity, evidence identifiers, derived indicators and decision impacts.

`BusinessEvidenceClaim` extends the numeric evidence register to money, ratios, counts, dates, categories, text, structures, events and projects. Every claim keeps source hash, page, geometry, reporting period, publication and availability dates, extraction method, review state, reviewer and restatement lineage, evidence tier, claim class and affected Business Twin domains. Pending or rejected claims cannot support an eligible banker-facing path.
...
Eighteen interpretable problem detectors combine deterministic rules, V3 wallet/PU/change-point evidence and governed scenario updating. Positive and disconfirming evidence are retained separately. A problem intensity is an interval rather than a bare score. In the absence of adjudicated labels, probability-like outputs are explicitly governed scenarios rather than empirically calibrated posteriors.

Stakeholders are ten role personas, never inferred named people. A governed responsibility matrix resolves a primary role, secondary roles, ownership rationale and an RM-confirmation requirement. In a bank deployment, CRM resolution may occur only after object-level authorization; the LLM cannot infer or publish named contacts.

All sixteen solution families execute for all twenty clients. The original Collections, Payments, Liquidity, Cross-border FX and Trade Finance solutions retain V3 bounds and posterior engines. Eleven adjacent solutions model solution-appropriate quantities: supply-chain finance, guarantees, working-capital/revolving credit, term/syndicated lending, DCM, project finance, interest-rate risk management, commodity risk management, escrow/agency, sustainable finance, and M&A/advisory. Each projection either returns a need/amount/timing/value result or a fail-closed reason. Project finance, sustainable finance and M&A/advisory currently fail closed for every client because critical inputs are absent. New families remain `SCENARIO` until empirical calibration and approved economics gates pass.

### Decision engineering

V3.1 computes a client frontier inside each client/problem and a portfolio frontier across entitled conversations. Robust dominance requires no worse client value, bank value, need and timing, no worse risk/friction, and one strict improvement in at least 80% of common scenario draws. Only non-dominated candidates are presented to the coverage optimizer.

The weekly policy combines need, client value, bank value, timing, relationship value and strategic value. It then applies feasibility, risk and implementation-friction multipliers. The mixed-integer objective blends 45% mean adjusted benefit with 55% lower-tail CVaR at 10%. It selects at most eight conversations, two per client, one per client/role, three per solution family and three per sector, while excluding failed feasibility and mutually exclusive bundles. Solver status and degraded fallback are explicit. The current plan is `OPTIMAL`; no degraded greedy fallback was used.

All selected actions are discovery conversations. Compliance/conduct and operations/onboarding are unknown across the demonstration boundary, with technology integration additionally unknown for integration-heavy solutions. Unknown gates cannot be turned into `PASS` by a model. The banker-facing action is therefore to establish facts, not to propose a product.

### Dual value and why-now

Client value and bank value are different ledgers. Client value may include monetised, proxy, qualitative and unavailable components; risk-reduction outputs never promise P&L savings. Bank value deducts FTP, liquidity, expected loss, capital, hedging, execution, servicing, operating cost and cost to win when approved inputs exist. Three-year relationship value remains separate from direct opportunity contribution to prevent double counting. Demo policies are representative and watermarked. Causal incremental value is null.

Timing combines the existing 30/60/90-day baseline, BOCPD signals, maturities, published events, project/tender dates, seasonality and solution lead times. If no supported event exists, the brief explicitly says that no time-critical trigger is established. V3.1 does not manufacture urgency.

### Active Coverage Learning

For each governed unknown, V3.1 estimates whether asking a question can change rank, bundle, feasibility or abstention. Value of information is computed with 512 common draws and explicit answer-state distributions:

```text
VOI(Q) = E[max U(a | Answer(Q))] - max E[U(a)] - Cost(Q) - Delay(Q)
```

Only positive-net-VOI questions with a decision effect are selected. A response creates an immutable, pending E2 candidate with respondent, consent, scope and validity. Pending answers do not alter approved evidence or models. Approval creates a new point-in-time evidence version and triggers a rebuild of the twin, graph, intervals and weekly plan, retaining the before/after decision impact.

### Runtime, persistence and workbench

V3.1 adds an eleventh deployable service boundary, `business_twin_app`, for Business Twin snapshots, graphs, events, problems, stakeholder resolution and funding routes. Existing recommendation and BFF services expose additive `/v3` contracts. The canonical API is version 3.1.0; `/v1` and V3.0 response shapes remain available. V3.0 is frozen as a regression asset at declared public precision.

The platform definitions now include governed Delta products for Business Twin components/indicators, graph nodes/edges, events, problems, stakeholders, solution estimates, funding routes, dual value, feasibility, conversations, Pareto results, coverage plans/draws, questions/VOI, approved answers, digests and outcomes. PostgreSQL definitions own publication workflow, business-claim review, coverage-plan state, feasibility attestations, answer review and a transactional outbox. Four MSK domain topics carry the event type in a versioned envelope. MLflow promotion policy registers the ontology, policies, all solution estimators, formulas, funding routes, graph extraction, CVaR, VOI and GenAI evaluation artifacts independently.

The workbench opens with “Your five most valuable conversations this week,” exposes all eight selected actions, and fetches one entitled conversation or client at a time. Client drilldowns show changes, all twelve Business Twin components, graph provenance, problem/counter-evidence, stakeholder rationale, Pareto status, separate value, six gates, timing, evidence, the primary question and the deterministic brief. The browser does not receive the full portfolio decision objects.

### Release conclusion

V3.1 is client-demonstration complete and bank-production `NOT_PROMOTABLE`. Code, public evidence and representative simulations cannot close the external gates: E3 multibank calibration, approved bank economics, completion of finance-SME review, bank identity/cloud/catalogue/SIEM operation, independent security and provider adjudication, empirical funding-route/problem validation, qualified RM outcomes, a supervised pilot, a randomized trial and thirty clean shadow days.

## Document control

| Field | Value |
|---|---|
| Document owner | Corporate Wallet Digital Twin V3.1 programme |
...
| Version | 3.1 system dossier â€” decision-lab implementation snapshot |
| Source of truth | The V3 repository, exported contracts, governed data registers and machine-generated validation outputs as at 12 August 2026 |
...
| Not promotable | One or more mandatory gates are absent or failed. The current V3.1 bank release is intentionally in this state. |
| Measured share | Wallet share supported by E3 multibank observation or E4 reconciliation. E0â€“E2 estimates are never given this label. |
...
Corporate Wallet Digital Twin V3.1 is a governed decision system for a partially observed corporate financial network. For each corporate relationship and product, it combines Syn Bank observed activity, point-in-time public evidence, assumption-light bounds, posterior wallet estimates, anonymous external-flow reconstruction, positiveâ€“unlabelled product-need estimates, Bayesian temporal-change signals, governed commercial scenarios, robust portfolio selection and decision-directed evidence acquisition. Its purpose is to help a relationship manager and control functions answer six separate questions:
...
- Three complete but nonbank E0 economics packs â€” conservative, reference and upside â€” and a fail-closed engine that blocks missing, expired, unreconciled or unapproved production inputs.
...
The controlled language forbids â€œmeasured competitor share,â€ â€œcausal upliftâ€ and â€œoptimal target shareâ€ in the current release. The output distinguishes action rationale from evidence gaps so that a banker can see both why an item ranks and what would most change the decision.
...
V3 can represent prior-led, publicly anchored, client-validated and empirically calibrated estimates. Only E3/E4 evidence can support the label â€œmeasured share.â€ No competitor transaction or actual competitor wallet-share data is present in the current repository. The workbench therefore shows inferred/posterior share and missing-evidence notices, not named competitor claims.
...
Contestable activity is `max(target_share Ã— wallet_median âˆ’ observed_activity, 0)`, capped by configured capacity. Scenario contribution applies the approved net rate and acquisition/implementation cost. A target share is called a scenario frontier, never â€œoptimal,â€ until response curves or causal win probabilities have been validated.
...
The transparent baseline uses a daily hazard of `0.0025 Ã— seasonal_ratio Ã— (0.6 + 0.8 Ã— recurrence)`. The seasonal ratio is clipped between 0.25 and 3.0. A known maturity inside 90 days increases the hazard by `1 + 1.5 Ã— (1 âˆ’ days_to_maturity / 90)`. The cumulative probability at a horizon is `1 âˆ’ exp(âˆ’daily_hazard Ã— days)`, ensuring monotonic 30/60/90-day probabilities.
...
```text
eligibility â†’ assignment â†’ display â†’ open/dismiss â†’ banker action
            â†’ pipeline milestone â†’ reconciled outcome
```
...
The trial analogue contains 1,500 opportunities in 30 clusters, with 387 30-day actions, 356 60-day milestones and 192 reconciled 90-day outcomes. A separate production-analysis rehearsal contains 1,152 events across 48 clusters; 1,042 are observed and 9.55% are censored. The ITT estimate is 0.00816 with cluster standard error 0.02425, 95% interval âˆ’0.0394 to 0.0557 and p-value 0.7365. The first stage is 0.50587 and the rehearsal Wald estimate is 0.01613. An A/A test produces ITT 0.00235 and randomization p-value 0.914.

These values confirm that the analysis code behaves sensibly, including when the correct conclusion is â€œno detectable effect.â€ They do not establish efficacy because the data are simulated. Production causal and uplift claims remain false.
...
## 20.3 Definition of â€œfull productionâ€
...
# Appendix B â€” JSON Schema catalogue
...
| `calibration-observation` | E2â€“E4 calibration evidence, selection probability/weight and source metadata |
...
# Appendix C â€” Public evidence coverage by client
...
| E0â€“E4 | Governed evidence-quality tiers defined in Section 6 |
...
# Appendix G â€” Mandatory next decisions
