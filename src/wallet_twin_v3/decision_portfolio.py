...
class RobustPortfolioOptimizer:
    ...
    def optimize(
        ...
        for opportunity in opportunities:
            ...
            conversion = rng.beta(
                3.0 + 4.0 * need.product_need_probability, 4.0, size=self.draws
            )
            leakage_urgency = 1.0 + leakage.alarm_probability * rng.uniform(
                0.0, 0.25, size=self.draws
            )
            scenario_values = np.maximum(
                0.0, base_value * wallet_factor * conversion * leakage_urgency
            )
            expected = float(scenario_values.mean())
            tail_count = max(1, int(self.draws * self.alpha))
            downside_cvar = float(np.sort(scenario_values)[:tail_count].mean())
            robust_score = (
            ...
            candidates.append(
                (
                    PortfolioAction(
                        ...
                        need_probability=need.product_need_probability,
                        leakage_probability=leakage.alarm_probability,
        ...
        portfolio_draws = (
            np.sum(selected_draws, axis=0) if selected_draws else np.zeros(self.draws)
        )
        tail_count = max(1, int(self.draws * self.alpha))
        return RobustActionPortfolio(
            ...
            expected_scenario_value_zar=float(portfolio_draws.mean()),
            downside_cvar_zar=float(np.sort(portfolio_draws)[:tail_count].mean()),
