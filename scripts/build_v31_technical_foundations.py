...
import build_v31_system_dossier as base  # noqa: E402


SOURCE = ROOT / "docs" / "Corporate_Wallet_Digital_Twin_V3_1_Technical_Foundations.md"
OUTPUT = ROOT / "deliverables" / "Corporate_Wallet_Digital_Twin_V3_1_Technical_Foundations.docx"
...
def add_cover(doc: Document) -> None:
    ...
    r = p.add_run(
        "A literature-grounded specification of the Business Model Twin, problem and stakeholder policies, "
        "sixteen solution estimators, wallet inference, robust Pareto/CVaR planning, decision-focused "
        "value of information, controlled GenAI and production engineering."
    )
    r.font.name = "Calibri"
    r.font.size = Pt(12)
    ...
    values = [
        ...
        ("Version", "3.1.0"),
        ("Technical baseline", "Corporate Banking Decision Twin V3.1"),
        ("As of", "12 August 2026"),
        ("Release truth", "V3.1 Decision Twin ready / bank production not promotable"),
    ]
...
def add_running_header_footer(section) -> None:
    ...
    def populate_footer(footer):
        ...
        r = p.add_run("LITERATURE-GROUNDED DESIGN  â€¢  10 AUGUST 2026     ")
        r.font.name = "Calibri"
        r.font.size = Pt(8)
        r.font.color.rgb = RGBColor.from_string(base.MUTED)
...
def build_architecture_diagram(path: Path) -> None:
    img = Image.new("RGB", (1900, 1040), base.pil_color(base.WHITE))
    draw = ImageDraw.Draw(img)
    draw.text((80, 48), "Corporate Wallet Digital Twin V3 â€” target production flow", font=base.font(40, True), fill=base.pil_color(base.NAVY))
    ...
    text_map = {
        ...
        "lake": ["S3 + Delta Lake", "Raw â†’ conformed â†’ curated", "Point-in-time features", "Object-locked snapshots"],
...
def build_ladder_diagram(path: Path) -> None:
    ...
    draw.text((80, 98), "Stronger evidence permits stronger labels â€” never automatic promotion", font=base.font(24), fill=base.pil_color(base.MUTED))
    ...
    draw.text((82, 828), "Observed â†’ identified bound â†’ posterior â†’ scenario â†’ causal", font=base.font(23, True), fill=base.pil_color(base.NAVY))
    img.save(path, quality=95)
...
def scrub_metadata(doc: Document) -> None:
    ...
    props.title = "Corporate Wallet Digital Twin V3.1 - Technical Foundations, Statistical Theory and Production Engineering"
    props.subject = "Literature-grounded V3.1 technical design specification as at 12 August 2026"
