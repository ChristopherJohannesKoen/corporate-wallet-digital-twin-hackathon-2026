...
const workbookPath = path.resolve(import.meta.dirname, "../outputs/audit/Public-Facts-Anchor-Register-V3.1.1.xlsx");
const workbook = await SpreadsheetFile.importXlsx(await FileBlob.load(workbookPath));
const sheetNames = workbook.worksheets.items.map((sheet) => sheet.name);
const expectedSheets = ["Cover", "Public Facts", "Client Coverage", "Approved Anchors", "Wallet Decision Impact", "Evidence Queue", "Sensitivity", "Checks", "Sources"];
const errors = [];
