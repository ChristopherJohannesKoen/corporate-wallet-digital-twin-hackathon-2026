...
const expectedSheets = ["Cover", "Public Facts", "Client Coverage", "Approved Anchors", "V3 Decision Impact", "Evidence Queue", "Sensitivity", "Checks", "Sources"];
const errors = [];
...
const checks = workbook.worksheets.getItem("Checks").getRange("A4:F18").values;
...
if (checks.slice(1).some((row) => row[3] !== "PASS")) throw new Error(`Workbook checks failed: ${JSON.stringify(checks)}`);
