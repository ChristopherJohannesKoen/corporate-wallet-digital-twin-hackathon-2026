...
SOURCE = ROOT / "docs" / "Corporate_Wallet_Digital_Twin_V2_System_Dossier.md"
OUT_DIR = ROOT / "deliverables"
ASSET_DIR = OUT_DIR / "assets"
OUTPUT = OUT_DIR / "Corporate_Wallet_Digital_Twin_V2_System_Dossier.docx"
...
def build_architecture_diagram(path: Path) -> None:
    ...
    draw.text((80, 48), "Corporate Wallet Digital Twin V2 — target production flow", font=font(40, True), fill=pil_color(NAVY))
    ...
    centered_lines(draw, boxes["lake"], ["S3 + Delta Lake", "Raw → conformed → curated", "Point-in-time features", "Object-locked snapshots"])
    centered_lines(draw, boxes["models"], ["Decision services", "Bounds + posterior wallet", "Economics + sensitivity", "30/60/90-day timing"])
...
def build_ladder_diagram(path: Path) -> None:
    ...
    draw.text((80, 98), "Stronger evidence permits stronger labels — never automatic promotion", font=font(24), fill=pil_color(MUTED))
    tiers = [
    ...
    draw.text((82, 828), "Observed → identified bound → posterior → scenario → causal", font=font(23, True), fill=pil_color(NAVY))
    img.save(path, quality=95)
...
def add_field(paragraph, instruction: str):
    ...
    text.text = "Update field in Word if page numbers are not visible."
...
def add_running_header_footer(section):
    def populate_header(header):
        ...
        r = p.add_run("CORPORATE WALLET DIGITAL TWIN V2  /  SYSTEM DOSSIER")
        r.font.name = "Calibri"
        r.font.size = Pt(8)
        r.font.bold = True
    ...
    def populate_footer(footer):
        ...
        r = p.add_run("IMPLEMENTATION SNAPSHOT  •  9 AUGUST 2026     ")
        r.font.name = "Calibri"
        r.font.size = Pt(8)
        r.font.color.rgb = RGBColor.from_string(MUTED)
        add_field(p, "PAGE")
...
def add_cover(doc: Document):
    ...
    r = p.add_run("STANDARD BANK HACKATHON 2026  /  V2 SYSTEM RECORD")
    r.font.name = "Calibri"
    r.font.size = Pt(9)
    r.font.bold = True
    ...
    r = p.add_run("A controlled record of the product, data, evidence, models, economics, timing, causal design, GenAI, interfaces, security, cloud target, workbench, validation and production handoff implemented to date.")
    r.font.name = "Calibri"
    r.font.size = Pt(12)
    ...
    values = [
        ...
        ("As of", "9 August 2026"),
        ("Release truth", "Client demo READY  /  Bank production NOT_PROMOTABLE"),
    ]
...
def add_markdown_table(doc, rows):
    ...
    for r_idx, row_values in enumerate(rows):
        ...
        for c_idx, value in enumerate(row_values):
            ...
            add_inline(p, value.strip(), color=WHITE if r_idx == 0 else INK, size=8.0 if len(rows[0]) >= 4 else 8.5)
            for run in p.runs:
                run.font.name = "Calibri"
                if r_idx == 0:
                    run.font.bold = True
...
def scrub_metadata(doc: Document):
    ...
    props.title = "Corporate Wallet Digital Twin V2 — Complete System Dossier"
    props.subject = "Authoritative implementation snapshot as at 9 August 2026"
    props.author = "Corporate Wallet Digital Twin V2 programme"
    ...
    props.last_modified_by = "Corporate Wallet Digital Twin V2 programme"


def main():
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    ASSET_DIR.mkdir(parents=True, exist_ok=True)
    build_architecture_diagram(ASSET_DIR / "v2_target_architecture.png")
    build_ladder_diagram(ASSET_DIR / "v2_evidence_claim_ladder.png")
