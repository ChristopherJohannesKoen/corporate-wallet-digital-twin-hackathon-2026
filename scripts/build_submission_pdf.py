...
W, H = A4
c.setTitle("Corporate Wallet Digital Twin V3.1.1 — Hackathon One-Pager")
...
c.drawString(32, H - 24, "STANDARD BANK HACKATHON 2026  /  V3.1.1")
...
c.setFillColor(INK)
c.drawString(left + 13, ly + 94, "Trade Finance is first-ranked in 100% of 10,000 global draws; portfolio dominance is not universal.")
c.setFont("Helvetica", 6.2)
c.setFillColor(MUTED)
c.drawString(left + 13, ly + 79, "E1 weight 0.20 / 0.35 / 0.50: known-truth coverage 88.8% / 88.0% / 86.8%; top-10 TF share 70% / 60% / 30%; majority 1 / 1 / 0.")
c.setStrokeColor(LINE)
c.line(left + 13, ly + 65, left + cw - 13, ly + 65)
label(c, "LIMITATION — SUBMISSION GATES ARE DISTINCT FROM BANK PRODUCTION", left + 13, ly + 49, AMBER)
draw_wrap(c, "The 51 pending facts remain excluded. No E3 multibank panel, approved bank pricing/FTP/capital inputs, bank cloud/identity controls, live-provider adjudication, RM pilot or randomized outcome history exists. Therefore bank status remains NOT_PROMOTABLE; no measured competitor share or causal uplift is claimed.", left + 13, ly + 34, cw - 26, size=6.2, max_lines=3)
...
c.drawString(left, 25, "Version 3.1.1  |  Data boundary: confidential Syn Bank activity + approved public E1 + governed priors  |  FX and Liquidity are labelled proxies")
c.drawRightString(W - left, 12, "Corporate Wallet Digital Twin · Christopher Koen")
c.showPage()
c.save()
print(json.dumps({"status": "ok", "output": str(OUTPUT), "pages": 1, "version": "3.1.1"}, indent=2))
