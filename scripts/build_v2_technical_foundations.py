...
"""Build the literature-grounded V2 technical foundations white paper.

The document uses the narrative-proposal preset from the document skill and
reuses the hardened OOXML utilities from the system-dossier builder.
"""
...
import build_v2_system_dossier as base  # noqa: E402


SOURCE = ROOT / "docs" / "Corporate_Wallet_Digital_Twin_V2_Technical_Foundations.md"
OUTPUT = ROOT / "deliverables" / "Corporate_Wallet_Digital_Twin_V2_Technical_Foundations.docx"
ASSET_DIR = ROOT / "deliverables" / "assets"


def configure_styles(doc: Document) -> None:
    """Apply the exact narrative_proposal typography tokens."""
    styles = doc.styles
...
def add_cover(doc: Document) -> None:
    ...
    r = p.add_run(
        ...
        "wallet inference, economics, uncertainty propagation, timing, causal learning, "
        "controlled GenAI, evidence governance and AWS–Databricks production target."
    )
    r.font.name = "Calibri"
    r.font.size = Pt(12)
    ...
    values = [
        ...
        ("Technical baseline", "Corporate Wallet Digital Twin V2"),
        ("As of", "9 August 2026"),
        ("Release truth", "Client demonstration ready / bank production not promotable"),
    ]
...
def add_running_header_footer(section) -> None:
    def populate_header(header):
        ...
        r = p.add_run("CORPORATE WALLET DIGITAL TWIN V2  /  TECHNICAL FOUNDATIONS")
        r.font.name = "Calibri"
        r.font.size = Pt(8)
        r.font.bold = True
        r.font.color.rgb = RGBColor.from_string(base.MUTED)
    ...
    def populate_footer(footer):
        ...
        r = p.add_run("LITERATURE-GROUNDED DESIGN  •  9 AUGUST 2026     ")
        r.font.name = "Calibri"
        r.font.size = Pt(8)
        r.font.color.rgb = RGBColor.from_string(base.MUTED)
        base.add_field(p, "PAGE")
        p.add_run(" / ")
        base.add_field(p, "NUMPAGES")
...
def add_toc(doc: Document) -> None:
    ...
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    table.autofit = False
    base.set_table_width(table, [4560, 4560])
    base.repeat_table_header(table.rows[0])
...
def add_note(doc: Document, text: str) -> None:
    ...
    table.autofit = False
    base.set_table_width(table, [9120])
    base.repeat_table_header(table.rows[0])
    cell = table.cell(0, 0)
    base.set_cell_shading(cell, "FFF8E7")
...
def build_architecture_diagram(path: Path) -> None:
    img = Image.new("RGB", (1900, 1040), base.pil_color(base.WHITE))
    draw = ImageDraw.Draw(img)
    draw.text((80, 48), "Corporate Wallet Digital Twin V2 — target production flow", font=base.font(40, True), fill=base.pil_color(base.NAVY))
    ...
    text_map = {
        ...
        "models": ["Decision services", "Bounds + posterior wallet", "Economics + sensitivity", "30/60/90-day timing"],
...
def scrub_metadata(doc: Document) -> None:
    ...
    props.title = "Corporate Wallet Digital Twin V2 — Technical Foundations, Statistical Theory and Production Engineering"
    props.subject = "Literature-grounded V2 technical design specification as at 9 August 2026"
...
def main() -> None:
    ...
    build_inference_diagram(ASSET_DIR / "v2_wallet_inference.png")
    build_learning_diagram(ASSET_DIR / "v2_causal_learning.png")
