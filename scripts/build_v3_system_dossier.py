...
SOURCE = ROOT / "docs" / "Corporate_Wallet_Digital_Twin_V3_System_Dossier.md"
OUT_DIR = ROOT / "deliverables"
ASSET_DIR = OUT_DIR / "assets"
OUTPUT = OUT_DIR / "Corporate_Wallet_Digital_Twin_V3_System_Dossier.docx"
...
def build_architecture_diagram(path: Path) -> None:
    ...
    draw.text((80, 48), "Corporate Wallet Digital Twin V3 — target production flow", font=font(40, True), fill=pil_color(NAVY))
...
def add_running_header_footer(section):
    ...
    def populate_footer(footer):
        ...
        r = p.add_run("IMPLEMENTATION SNAPSHOT  •  10 AUGUST 2026     ")
        r.font.name = "Calibri"
        r.font.size = Pt(8)
        r.font.color.rgb = RGBColor.from_string(MUTED)
        add_field(p, "PAGE")
...
def add_cover(doc: Document):
    ...
    r = p.add_run("STANDARD BANK HACKATHON 2026  /  V3 SYSTEM RECORD")
    r.font.name = "Calibri"
    r.font.size = Pt(9)
    r.font.bold = True
    ...
    r = p.add_run("The authoritative record of the governed wallet substrate, latent-network reconstruction, uncertainty-aware decision layer, controlled GenAI, production engineering, validation and bank handoff.")
    r.font.name = "Calibri"
    r.font.size = Pt(12)
    ...
    values = [
        ...
        ("Release truth", "V3 Decision Lab READY / Bank production NOT_PROMOTABLE"),
    ]
...
def scrub_metadata(doc: Document):
    ...
    props.title = "Corporate Wallet Digital Twin V3 — Complete System Dossier"
    props.subject = "Authoritative implementation snapshot as at 10 August 2026"
